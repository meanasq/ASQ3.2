...
app.get('/activateMe',function(req, res) {
    ...
    if (!doc) return res.send(500, { error: 'EMAIL NOT FOUND !' });
	else {
		  
		userModel.update({email: theEmail}, {
    		activeIn: 'Y'    
 		}, function(err, numberAffected, rawResponse) {
   
  });
		
		console.log("Found : "+doc.email);
		...
		email : req.body.email
			...
			if (result)
				...
				var data = {
					...
					url 	 : "http://"+req.headers.host+"/activateMe?email="+encrypt(user.email), //edited by DIYA
					name 	 : user.firstName
				}
