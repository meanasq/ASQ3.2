	...
	$rootScope.$on('$routeChangeStart',	function (event, next, current) {
		...
		AppFunctions.UserStatus().then(function() {
			...
			if( $rootScope.currentUser.activeIn === 'N') {
				AppFunctions.showAlert("ACCOUNT ACTIVATION ERROR !!", "Account not Activated. Please activated your account by clicking on activation link sent to your email: " + $rootScope.currentUser.email);
				AppFunctions.logout();
			...
			else if ( next.access.restricted && !AppFunctions.userLoggedin ) {
...
app.controller('registerController', 
    ...
    $scope.testUsername = function () {
        ...
        if($scope.user.email != "") {
		   ...
		   if($scope.usernameErr==true){$scope.error=true;}
				else $scope.error=false;
        } 
        else {
           $scope.usernameErr = false;
		   if( $scope.zipCodeErr || $scope.usernameErr || 
		   	   $scope.birthDateErr ||$scope.passwordShort || 
		   	   $scope.passwordErr ||$scope.checkEmail) {
    ...
    //Check Zip Code
 	$scope.testZipCode = function(){
		...
		if($scope.user.zipcode != "") {
			...
			//alert('X : '+re.test($scope.user.zipcode));
			if($scope.zipCodeErr==true)
				$scope.error=true;
			else 
				$scope.error=false;
		} 
				...
				.success(function (response) {
					if (response != "0") {
						AppFunctions.showAlert("CONGRATULATIONS !!",
							"Your Account has been created! Activation link sent to your email \"" + user.email + 
...
// ------------------------------------------- BEGIN TIMER SECTION ---------------------------------------------
app.controller('timerController', function($scope, ObserverService, $location) {	
	$scope.$on('timer-stopped', function () {
		// console.log('notify');
		ObserverService.notify('timeUp','timer');
	});

	$scope.getshit = function(item) {
		console.log("element = " + document.getElementByID("myTimer"));
		console.log("value = " + item.currentTarget.getAttribute("id"));
	}
});
...
app.controller('examController', 
	...
	$scope.template = { "timerPage" : "pages/timer.html" };
...
app.controller('historyController', 
	function ($q, $scope, $rootScope, $http, $location, $route, AppFunctions) {
